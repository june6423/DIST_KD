...
from .dist_kd import DIST, DIST_plus
...
class KDLoss():
    ...
    def __init__(self, student, teacher, ori_loss, kd_method='kdt4', student_module='', teacher_module='', ori_loss_weight=1.0, kd_loss_weight=1.0):
        ...
        if kd_method == 'kd':
            ...
            self.kd_loss = DIST_plus(beta=1, gamma=1, tau=1)
    ...
    def __call__(self, x, targets):
        ...
        # compute kd loss
        topk_values, topk_indices = torch.topk(self._teacher_out, k=10, dim=1)
        mask = torch.zeros_like(self._teacher_out, dtype=torch.bool)
        mask.scatter_(1, topk_indices, True)
        self._teacher_out = torch.where(mask, self._teacher_out, torch.full_like(self._teacher_out, float(0)))
